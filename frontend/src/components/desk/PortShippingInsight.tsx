...
async function requestPortInsight(): Promise<{
  insight?: string;
  error?: string;
  missingKey?: boolean;
}> {
  const res = await fetch("/api/port-insight", {
    method: "POST",
...
export function PortShippingInsight() {
  ...
  const load = useCallback(async () => {
    ...
    setMissingKey(false);
    try {
      const { insight, error: err, missingKey: noKey } = await requestPortInsight();
      if (err) {
        setError(err);
        setMissingKey(Boolean(noKey));
      } else setText(insight ?? null);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(94,106,210,0.1)_0%,transparent_55%)] opacity-90"
        aria-hidden
      />
        ...
        <div className="mt-4 min-h-[3rem]">
          {loading && !text && (
            <p className="text-sm leading-relaxed text-foreground-subtle">
              Scanning Houston, NY/NJ, and Long Beach…
            </p>
          )}
          {error && (
            <p
